...
import { Menu, Bell, Search, SlidersHorizontal, ChevronRight, TrendingUp, Sparkles, Flame } from "lucide-react";
...
export default function Home({ dataSource }: HomeProps) {
    ...
    const fetchHomeData = async () => {
      ...
      try {
        ...
        if (active) {
          ...
          setRecent(homeData.recent || homeData.recents || []);
  ...
  // Navigate to Explore page with direct settings
  ...
  return (
    ...
    >
      ...
      <div className="mb-5 overflow-hidden">
        ...
        <div className="flex gap-2 overflow-x-auto px-5 pb-2 scrollbar-none">
          <button 
            ...
            className="bg-[#121319]/80 border border-white/5 text-[#a0a5b5] hover:text-white rounded-2xl text-xs px-4 py-2 flex items-center gap-1.5 flex-shrink-0 cursor-pointer"
          >
            <span>All</span>
          </button>
          <button 
        ...
        <div className="flex gap-2 overflow-x-auto px-5 scrollbar-none">
          ...
          <button 
            onClick={() => handleChipClick("Genres", "sci-fi")}
            className="bg-[#121319]/80 border border-white/5 text-[#a0a5b5] hover:text-white rounded-2xl text-xs px-4 py-2 flex items-center gap-1.5 flex-shrink-0 cursor-pointer"
          >
            <span>🚀 Futuristic</span>
          </button>
          <button 
            onClick={() => handleChipClick("Genres", "comedy")}
            className="bg-[#121319]/80 border border-white/5 text-[#a0a5b5] hover:text-white rounded-2xl text-xs px-4 py-2 flex items-center gap-1.5 flex-shrink-0 cursor-pointer"
          >
            <span>😊 Comedy</span>
          </button>
      ...
      {/* 5. SECTION "New Releases" */}
      <div className="mb-6">
        <div className="flex justify-between items-center px-5 mb-3">
          <h2 className="text-base font-semibold text-white font-sans">New Releases</h2>
          <button 
            onClick={() => handleSeeAll("Latest")}
            className="flex items-center text-xs text-[#535766] hover:text-[#a0a5b5] cursor-pointer font-sans"
          >
            See all
